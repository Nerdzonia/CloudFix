...
const TicketList = props => {
  ...
  const handleSort = async clickedColumn => {
    ...
    if (column !== clickedColumn) {
      ...
      await ticketRequestor
        .getAllTickets({ ...paginateRule, query })
        .then(ticket => {
          setSort({
  ...
  const handleSendQuery = async () => {
    ...
    Object.keys(inputs).map(e =>
      inputs[e] !== "" ? (newQuery[e] = inputs[e]) : null
    );
    setQuery({ ...newQuery });
    await ticketRequestor
      .getAllTickets({ query: { ...newQuery } })
      .then(ticket => {
        setSort({
          column: null,
          data: ticket.data.docs || [],
  ...
  const updateTicketStatus = async (id, status) => {
    ...
    await ticketRequestor
      .getAllTickets({ pagination: { ...paginateTicket }, query })
      .then(ticket => {
        setSort({
          ...sort,
          data: ticket.data.docs || []
      ...
      <Table.Cell>
        <Button.Group>
          <Button
            ...
            icon="check circle"
            color="green"
            onClick={() => updateTicketStatus(id, "solved")}
                        ...
                        >
                          <Table.Header>
                            <Table.Row>
                              ...
                              <Table.HeaderCell
                                sorted={
                                  column === "updatedAt" ? direction : null
                                }
