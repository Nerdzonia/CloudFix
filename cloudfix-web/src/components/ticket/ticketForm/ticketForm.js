...
const TicketForm = props => {
  ...
  const handleFildsChange = (e, { name, value }) => {
    ...
    if (check[name]) setCheckInput({ ...checkInput, [name]: false });
  ...
  const handleImage = e => {
    if (e.target.files[0]) {
      ...
      if (file.size >= 2097152)
        return setAlert(
          <Alert
            ...
            message={"Porfavor insira um arquivo menor que 2MB"}
            open={true}
            title="Aviso"
      ...
      if (input.image.length >= 5)
        return setAlert(
          <Alert
            ...
            iconTitle="warning"
            iconButton="checkmark"
            message={"So pode carregar em ate arquivos."}
            open={true}
            title="Aviso"
            removeAlert={setAlert}
          />
      ...
      setInput({
        ...input,
        image: input.image ? [file, ...input.image] : [file]
      });
  ...
  const sendTicket = async () => {
    ...
    Object.keys(input).forEach(element => {
      if (element !== "image")
        if (input[element] == "") check[element] = true;
        else check[element] = false;
    });
  ...
  const ArchiveBox = () => (
    <Message>
      <Message.Header>
        Itens adicionados (Maximo de arquivos 5, tamanho ate 2MB)
      </Message.Header>
      <Message.List
        items={input.image.map((image, i) => (
          <div key={i}>
            <strong>{i + 1} == </strong> {image.name}
          </div>
        ))}
      />
    </Message>
          ...
          <Segment>
            <Grid.Row>
              <Header as="h1" textAlign="center">
                Solicitar Ticket
                <Header.Subheader>
                  Para solicitar um ticket, é necessário que informe seus dados,
                  e depois descrever o problema.
                </Header.Subheader>
              </Header>
              <Divider />
            </Grid.Row>
                      ...
                      <Grid.Row>
                        <Grid.Column>
                          <Form.Field required>
                            ...
                            <Form.Input
                              ...
                              error={
                                checkInput.title
                                  ? { content: "Por favor, insira um assunto!" }
                                  : null
                              }
                              name="title"
                              value={input.title}
                          ...
                          <Form.Field required>
                            ...
                            <SystemSelect
                              ...
                              errorMessage={
                                "Por favor, selecione o sistema que ocorreu o erro!"
                              }
                              value={input.system}
                              onChange={handleFildsChange}
                            />
                        ...
                        <Grid.Column>
                          ...
                          <Button.Group floated="right">
                            <Button onClick={() => setInput(inputInitialState)}>
                              Cancelar
                            </Button>
                            <Button.Or text="ou" />
                            <Button
